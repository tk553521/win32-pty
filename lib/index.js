...
async function apply(ctx) {
  ...
  // Save the original values so hot-unload can restore them exactly.
  const originalTerminalInspector = runtime.terminalInspector;
  const originalSpawnTerminal = runtime.spawnTerminal;

  // Track terminal handles whose signalForeground we patched, so they can be
  // restored if this plugin is hot-disabled.
  const patchedHandles = new Map();
  ...
  const patchedSpawnTerminal = async (spec) => {
    const fixed = fixShellPath(unwrapSandboxArgv(spec.argv));
    const handle = await originalSpawnTerminal.call(runtime, fixed === spec.argv ? spec : { ...spec, argv: fixed });
    if (!patchedHandles.has(handle)) {
      const originalSignalForeground = handle.signalForeground;
      const patchedSignalForeground = async (signal) => {
        if (signal === "SIGINT") {
        ...
        return originalSignalForeground.call(handle, signal);
      };
      handle.signalForeground = patchedSignalForeground;
      patchedHandles.set(handle, { original: originalSignalForeground, patched: patchedSignalForeground });
    }
    return handle;
  };

  // Cordis effect: the setup runs now, and the returned disposer runs when this
  // plugin is hot-disabled/reloaded, restoring every monkey-patch we made.
  ctx.effect(() => {
    runtime.terminalInspector = inspector;
    runtime.spawnTerminal = patchedSpawnTerminal;
    patched.add(runtime);
    ctx.logger?.info?.(
    ...
    return () => {
      if (runtime.spawnTerminal === patchedSpawnTerminal) {
        runtime.spawnTerminal = originalSpawnTerminal;
      }
      if (runtime.terminalInspector === inspector) {
        runtime.terminalInspector = originalTerminalInspector;
      }
      for (const [handle, { original, patched: patchedSignalForeground }] of patchedHandles) {
        if (handle.signalForeground === patchedSignalForeground) {
          handle.signalForeground = original;
        }
      }
      patchedHandles.clear();
      patched.delete(runtime);
      ctx.logger?.info?.("[win32-pty] disposed: restored original subprocess runtime");
    };
  }, "win32-pty.apply");
}
